...
# ---------------------------------------------------------------------------
# 初心者モード用のやさしいグレード呼称（label_simple）
# ---------------------------------------------------------------------------

class TestOverallSimpleLabel:
    """初心者モードでは分析結果のみ表示するため、解説文は持たず
    グレードのやさしい呼称(label_simple)だけを返すことを確認する"""

    def _scores(self, cores):
        s = PCSpecs()
        s.cpu_name, s.cpu_cores, s.cpu_threads, s.cpu_base_ghz = "CPU", cores, cores * 2, 3.0
        s.ram_total_gb, s.ram_type = 32.0, "DDR5"
        s.gpu_name, s.gpu_vram_gb = "NVIDIA GeForce RTX 5070", 12.0
        s.storage_list = [{
            "model": "SSD", "size_gb": 1000.0, "free_gb": 500.0,
            "is_ssd": True, "is_nvme": True, "mountpoints": ["C:\\"],
        }]
        profile = PROFILES["mid"]
        return [
            analyze_cpu(s, profile),
            analyze_ram(s, profile),
            analyze_gpu(s, profile),
            analyze_storage(s, profile),
        ]

    def test_overall_has_simple_label(self):
        overall = calculate_overall(self._scores(cores=16))
        assert overall["label_simple"]

    def test_no_simple_explanation_fields(self):
        # 解説文(message_simple)は削除済みであること
        overall = calculate_overall(self._scores(cores=16))
        assert "message_simple" not in overall
